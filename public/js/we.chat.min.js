WE.pageChat={isLoading:0,lastTime:null,init:function(){this.ui={postBox:$("#post-box")};this.regEvent()},regEvent:function(){var a=this;this.ui.postBox.find(".text-box input").keyup(function(b){b.keyCode==13&&(b=$.trim($(this).val()),a.post(ROOM.id,b))})},post:function(a,b,c){aim=!c?void 0:c;var c=new WE.api.ChatModel,d=new WE.Controller;d.update=function(a){a.data.code==0&&this.ui.postBox.find(".text-box input").val("")};c.addObserver(d);c.postChat(a,b,aim)},setReply:function(a){var b=$(MX.kit.tmpl(this.replyTmpl));
b.find(".close-btn").click(function(){b.remove()});console.log("chat=======",a);this.ui.postBox.empty().prepend(b)},replyTmpl:'<div class="replay-box">\t\t\t\t\t<div class="arrow"></div>\t\t\t\t\t<a href="javascript:;" class="icon-close"></a>\t\t\t\t\t<div>\t\t\t\t\t\t<a href="#" class="name">AeroJin</a>\t\t\t\t\t</div>\t\t\t\t\t<div class="context">\u50bb\u8d27</div>\t\t\t\t</div>'};
WE.pageChat.timeLine={tmpl:'<div class="talk <% if( from._id == USER._id ){ %> me <% } %>">\t\t\t\t<div class="photo">\t\t\t\t\t<a href="/user/<%=from._id%>" target="_blank" data-uid="<%=from._id%>">\t\t\t\t\t\t<img src="<%=from.avatar%>">\t\t\t\t\t</a>\t\t\t\t</div>\t\t\t\t<div class="info">\t\t\t\t\t<div class="head">\t\t\t\t\t\t<a href="/user/<%=from._id%>" target="_blank" class="name"><%=from.name%></a>\t\t\t\t\t\t<a target="_blank" href="/d/<%=_id%>" class="time">\t\t\t\t\t\t\t<i class="icon-clock"></i><%=WE.kit.format( new Date( time*1000 ),"MM-dd hh:mm:ss" )%>\t\t\t\t\t\t</a>\t\t\t\t\t</div>\t\t\t\t\t<% if(obj.aim){ %>\t\t\t\t\t<div class="reply"><%=aim.text%></div>\t\t\t\t\t<% } %>\t\t\t\t\t<div class="context">\t\t\t\t\t\t<%=WE.markdown.format(text)%><a href="javascript:;" data-mid="<%=_id%>" class="icon-reply" title="\u56de\u590d"></a>\t\t\t\t\t</div>\t\t\t\t</div>\t\t\t</div>',mapData:{},
