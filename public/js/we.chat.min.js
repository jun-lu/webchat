...
WE.pageChat.timeLine={tmpl:'<div class="talk <% if( from._id == USER._id ){ %> me <% } %>">\t\t\t\t<div class="photo">\t\t\t\t\t<a href="/user/<%=from._id%>" target="_blank" data-uid="<%=from._id%>">\t\t\t\t\t\t<img src="<%=from.avatar%>">\t\t\t\t\t</a>\t\t\t\t</div>\t\t\t\t<div class="info">\t\t\t\t\t<div class="head">\t\t\t\t\t\t<a href="/user/<%=from._id%>" target="_blank" class="name <% if(from.name==""){ %> no-name <% } %>"><%= from.name == ""? "(\u6682\u65e0\u6635\u79f0)" : from.name %></a>\t\t\t\t\t\t<a target="_blank" href="/d/<%=_id%>" class="time">\t\t\t\t\t\t\t<%=WE.kit.format( new Date( time*1000 ),"MM-dd hh:mm:ss" )%>\t\t\t\t\t\t</a>\t\t\t\t\t</div>\t\t\t\t\t<% if(obj.aim){ %>\t\t\t\t\t<div class="reply"><%=aim.text%></div>\t\t\t\t\t<% } %>\t\t\t\t\t<div class="context">\t\t\t\t\t\t<%=WE.markdown.format(text)%><a href="javascript:;" data-mid="<%=_id%>" class="icon-reply chat-reply" title="\u56de\u590d"></a>\t\t\t\t\t</div>\t\t\t\t</div>\t\t\t</div>',mapData:{},
