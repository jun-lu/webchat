...
init:function(a,b){var c=this,d=0,e=a.length;c.chats_count=b;for(c.leave_count=30<b?b-30:0;d<e;d++)this.mapData[a[d]._id]=a[d];this.appends(a);e&&a[0].time?WE.pageChat.lastTime=a[0].time:(WE.pageChat.isLoading=2,WE.pageChat.lastTime=-1);$("#timeline-talks").delegate(".chat-reply","click",function(){var a=$(this).attr("data-mid");(chat=c.mapData[a])&&WE.pageChat.setReply(chat)})},append:function(a){this.mapData[a._id]=a;$("#timeline-talks").append(WE.kit.tmpl(this.tmpl,a));$("#timeline-bar").scrollTop(99999)},
appends:function(a){for(var b=0,c=a.length,d="";b<c;b++)this.mapData[a[b]._id]=a[b],d+=WE.kit.tmpl(this.tmpl,a[b]);$("#timeline-talks").append(d);a[0]&&a[0].time?WE.pageChat.lastTime=a[0].time:console.log("lastTime \u5931\u8d25");$("#timeline-bar").scrollTop(99999)},prepends:function(a){for(var b=0,c=a.length,d="";b<c;b++)this.mapData[a[b]._id]=a[b],d+=WE.kit.tmpl(this.tmpl,a[b]);b=$(d);b.insertAfter($("#more-talks"));var e=0;$.each(b,function(a,b){e+=b.offsetHeight});$("#timeline-bar").scrollTop(e-
20);a[0].time?WE.pageChat.lastTime=a[0].time:console.log("lastTime \u5931\u8d25")}};
...
init:function(a){var b=a.length-1,c;if(a){this.data=a;for(c=""+WE.kit.tmpl(this.tmpl,USER);0<=b;b--)this.data[b]._id!=USER._id&&(c+=WE.kit.tmpl(this.tmpl,a[b]));$("#user-list").empty().html(c)}},append:function(a){if(a._id!=USER._id){this.data?this.data.push(a):this.data=[a];a.online=!0;var b=WE.kit.tmpl(this.tmpl,a);setTimeout(function(){$("#uid_"+a._id).removeClass("insert-li")},300);$("#user-list li").eq(0).after(b)}},remove:function(a){var b=this.data;$("#uid_"+a._id).remove();for(var c=0;c<b.length;c++)if(b[c]._id==
a._id){b.splice(c,1);break}}};
...
WE.pageChat.notice={isSound:!0,isWindowFocus:!0,pointer:null,init:function(){this.ui={boot:$("#notice-sound"),iconBoot:$("#notice-sound").find(".js-boot")};this.isSoundCookie=WE.cookie.getCookie("noticeSound");"0"==this.isSoundCookie&&(this.ui.iconBoot.removeClass("icon-onnoticesound").addClass("icon-offnoticesound"),this.isSound=!1);this.ui.boot.find(".js-loading").remove();this.ui.iconBoot.removeClass("hidden");this.sound=document.createElement("audio");this.sound.src="/public/msg.mp3";this.regEvent();
this.defaultTitle=document.title},regEvent:function(){var a=this;this.ui.boot.click(function(){a.isSound?a.isSound=!1:a.isSound=!0;a.isSound?WE.cookie.setCookie("noticeSound","1"):WE.cookie.setCookie("noticeSound","0");a.setBootUi(a.isSound)});$(window).click(function(){a.isWindowFocus=!0;document.title=a.defaultTitle;clearInterval(a.pointer)});$(window).blur(function(){a.isWindowFocus=!1})},setBootUi:function(a){this.ui.iconBoot.removeClass("icon-onnoticesound").addClass("icon-offnoticesound").attr("title",
"\u70b9\u51fb\u5f00\u542f\u65b0\u6d88\u606f\u58f0\u97f3\u63d0\u793a");a&&this.ui.iconBoot.removeClass("icon-offnoticesound").addClass("icon-onnoticesound").attr("title","\u70b9\u51fb\u5173\u95ed\u65b0\u6d88\u606f\u58f0\u97f3\u63d0\u793a")},onTitleNotice:function(a){var b=this;b.noticeTitle=a;clearInterval(this.pointer);if(!b.isWindowFocus){if(b.isSound)this.onSound();this.pointer=setInterval(function(){document.title==b.noticeTitle?document.title=b.defaultTitle:document.title=b.noticeTitle},1E3)}},
onSound:function(){this.sound.currentTime=0;this.sound.play()}};
WE.pageChat.invite={selectList:{mail:[],user:[]},datas:[],isGetData:!1,isSending:!1,mailInputList:[],init:function(){var a=$("#invite");this.ui={wall:$("#wall-room"),inviteWall:a,boot:$("#invite-btn"),close:a.find(".js-close-btn"),usersList:$("#users-list"),emailInput:a.find(".email-input"),emailTips:a.find(".email-input .error-tips"),submit:$("#invite-submit")};this.regEvent()},regEvent:function(){var a=this;this.ui.boot.click(function(){a.ui.wall.addClass("invite-style");a.isGetData?a.addUsers(a.datas):
a.getUserList()});this.ui.close.click(function(){a.ui.wall.removeClass("invite-style");a.ui.usersList.empty();a.ui.emailInput.find("input").val("");a.selectList.mail=[];a.selectList.user=[]});this.ui.usersList.find(".cell").click(function(){var a=$(this);a.index();var c=a.hasClass("select-cell");c?a.removeClass("select-cell"):a.addClass("select-cell");c?a.find(".select").addClass("hidden"):a.find(".select").removeClass("hidden");c?a.find(".invite").show():a.find(".invite").hide()});this.ui.emailInput.find("input").keyup(function(b){var c=
$.trim(a.ui.emailInput.find("input").val());if(13==b.keyCode&&""!=c){if(!/^[^@]+@[^@]+$/.test(c))return a.ui.emailTips.text("Please input email").show(),setTimeout(function(){a.ui.emailTips.hide()},2E3),!1;b=a.selectList.mail.length;for(var d=a.selectList.mail,e=!1,f=0;f<b;f++)if(d[f].mail==c){e=!0;a.ui.emailTips.text("Email already exists").show();setTimeout(function(){a.ui.emailTips.hide()},2E3);break}e||a.addUser({mail:c},!0);a.ui.emailInput.find("input").val("")}});this.ui.emailInput.find("input").focus(function(){a.ui.emailTips.hide()});
this.ui.submit.click(function(){if(a.selectList.mail.length||a.selectList.user.length)a.isSending||a.invitePost()})},addUser:function(a,b){(new WE.pageChat.inviteCell(a,b)).prepend(this.ui.usersList)},addUsers:function(a){for(var b=a.length,c=0;c<b;c++)this.addUser(a[c].to,!1)},getUserList:function(){var a=this,b=new WE.api.UserModel,c=new WE.Controller;c.update=function(b){b=b.data;0==b.code&&(a.isGetData=!0,a.datas=b.result,a.addUsers(a.datas))};b.addObserver(c);b.getContactList()},invitePost:function(){var a=
this;this.isSending=!0;this.ui.submit.text("Sending...");var b=new WE.api.RoomModel,c=new WE.Controller;c.update=function(b){a.isSending=!1;a.ui.submit.text("Send Invite");a.ui.emailInput.find("input").val("");a.selectList.user=[];a.selectList.mail=[];0==b.data.code?a.ui.emailInput.find(".invite-tips").text("Success").show():a.ui.emailInput.find(".invite-tips").text("Fail to send emails").show();a.ui.usersList.empty();setTimeout(function(){a.ui.emailInput.find(".invite-tips").hide()},3E3)};b.addObserver(c);
b.inviteChat(a.selectList.user,a.selectList.mail,ROOM.id)}};WE.pageChat.inviteCell=function(a,b){this.data=a;this.select=b||!1;this.list=(this.isUserList="undefined"!=typeof a.name?!0:!1)?WE.pageChat.invite.selectList.user:WE.pageChat.invite.selectList.mail;this.listIndex=this.list.length;var c=WE.kit.tmpl(this.itemTmpl,{user:this.data,select:b});wall=$(c);this.ui={wall:wall,invite:wall.find(".invite-js-btn"),cell:wall.find(".cell")};this.select&&this.addUser();this.init()};
WE.pageChat.inviteCell.prototype={itemTmpl:'<li>\t\t\t\t<div class="cell <% if( select ){ %> select-cell <% } %>">\t\t\t\t\t<% if( typeof user.avatar != "undefined" ){ %>\t\t\t\t\t<img src="<%=user.avatar %>">\t\t\t\t\t<% } %>\t\t\t\t\t<% if( typeof user.name != "undefined" && user.name == "" ){ %>\t\t\t\t\t\t<% user.name = "(\u672a\u8bbe\u7f6e\u6635\u79f0\u7684\u5c0f\u4f19\u4f34)"; %>\t\t\t\t\t<% } %>\t\t\t\t\t<%=user.name || user.mail %>\t\t\t\t\t<% if( select ){ %>\t\t\t\t\t\t<div class="invite-js-btn">\t\t\t\t\t\t\t<span class="select select-type">\u221a</span>\t\t\t\t\t\t\t<span class="btn invite-type invite" style="display:none;">\u9080\u8bf7</span>\t\t\t\t\t\t</div>\t\t\t\t\t<% }else{ %>\t\t\t\t\t\t<div class="invite-js-btn">\t\t\t\t\t\t\t<span class="select select-type hidden">\u221a</span>\t\t\t\t\t\t\t<span class="btn invite-type invite">\u9080\u8bf7</span>\t\t\t\t\t\t</div>\t\t\t\t\t<% } %>\t\t\t\t</div>\t\t\t</li>',
init:function(){this.regEvent()},regEvent:function(){var a=this;this.ui.cell.click(function(){a.select=!0==a.select?!1:!0;a.select?(a.ui.invite.find(".select-type").removeClass("hidden"),a.ui.invite.find(".invite-type").hide(),a.ui.cell.addClass("select-cell"),a.addUser()):(a.ui.invite.find(".select-type").addClass("hidden"),a.ui.invite.find(".invite-type").show(),a.ui.cell.removeClass("select-cell"),a.removeUser())})},prepend:function(a){a.prepend(this.ui.wall)},addUser:function(){this.isUserList?
this.list.push(this.data._id):this.list.push(this.data.mail)},removeUser:function(){for(var a=this.list.length,b=0;b<a;b++)this.isUserList?this.data._id&&this.data._id==this.list[b]._id&&this.list.splice(b,1):this.data.mail&&this.data.mail==this.list[b].mail&&this.list.splice(b,1)}};
WE.pageChat.roomEdit={init:function(){this.ui={boot:$("#room-edit-boot"),box:$("#room-edit-box"),titleBox:$("#title-box"),checkpwd:$("#checkpwd"),password:$("#password"),topic:$("#topic-name"),form:$("#room-edit-form")};this.regEvent()},regEvent:function(){var a=this;this.ui.boot.click(function(){a.clear();a.ui.titleBox.height("100%");a.ui.topic.focus();a.ui.box.show()});this.ui.box.find(".close-btn").click(function(){a.ui.titleBox.height("auto");a.ui.box.hide()});this.ui.box.find(".topic-edit-submit").click(function(){var b=
$.trim(a.ui.topic.val()),c=$.trim(a.ui.password.val());if(""!=b)return a.post(ROOM.id,b,c),!1});this.ui.checkpwd.change(function(){if(!a.ui.checkpwd.is(":checked"))return a.ui.password.hide(),a.ui.password.val(""),!1;a.ui.password.show()})},post:function(a,b,c){var d=new WE.api.RoomModel,e=new WE.Controller;e.update=function(a){0==a.data.code&&location.reload()};d.addObserver(e);d.updateRoomBase(a,b,c)},clear:function(){null==ROOM.password?this.ui.password.hide():this.ui.password.show();null!=ROOM.password?
this.ui.checkpwd.attr("checked","checked"):this.ui.checkpwd.removeAttr("checked");this.ui.password.val(null==ROOM.password?"":ROOM.password);this.ui.topic.val(ROOM.topic)}};
