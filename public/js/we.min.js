...
a)},setPos:function(a,c){this.ui.wrapper.css({top:a+"px",left:c+"px"})}};return function(b){return new a(b)}}()});WE.extend(WE.markdown,{defaultPrefix:"```",format:function(a){if(String(a).length>0){for(var a=a.split(this.defaultPrefix),b=-1,c=0;c<a.length;c++)b==-1?(b=c,a[c]=tool.removeHtmlTag(a[c]),a[c]=this.lineFormat(a[c]),a[c]=this.urlFormat(a[c])):(a[b+1]="<pre>"+a[b+1],a[c]+="</pre>",b=-1);return a.join("")}else return a},urlFormat:function(a){return a.replace(/(http|https):\/\/[\w\.\/\:\?\&\=\#\-\_]+/gi,function(a){return'<a href="'+a+'" target="_blank">'+a+"</a>"})},lineFormat:function(a){return a.replace(/\n/gi,
