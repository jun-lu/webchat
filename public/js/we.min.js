...
c=new WE.api.AtModel,d=new WE.Controller(c);d.update=function(a){b.showList(a.data)};c.addObserver(d);c.searchFirends(a||"")}};WE.Dialog=function(a){var b=null;if((this.id=a.id)&&WE.Dialog.getItem(this.id))return b=WE.Dialog.getItem(this.id),b.isRepeat=!0,b.isRepeat;this.html=a.html;this.width=a.width||0;this.height=a.height||0;this.wrap=$(WE.Dialog.html);this.ui={wrap:this.wrap,dialog:this.wrap.find(".we-dialog"),content:this.wrap.find(".we-dialog-context")};this.init()};WE.Dialog.html='<div class="we-dialog-box">\t\t<div class="we-mask"></div>\t\t<div class="we-dialog">\t\t\t<div class="we-dialog-title">\t\t\t\t<h1>Hello</h1>\t\t\t\t<a href="javascript:;" class="we-dialog-close out"></a>\t\t\t</div>\t\t\t<div class="we-dialog-context"></div>\t\t</div>\t</div>';
...
WE.Dialog.prototype={constructor:WE.Dialog,init:function(){var a=this;this.id&&WE.Dialog.setItem(this.id,this);this.ui.content.html(this.html);document.body.appendChild(this.ui.wrap[0]);this.width!=0?this.ui.dialog.width(this.width):this.width=this.ui.dialog.width();this.height!=0?this.ui.dialog.height(this.height):this.height=this.ui.dialog.height();this.ui.dialog.css({top:-this.height+"px",left:"50%",marginLeft:-(this.width/2)+"px"});this.ui.wrap.find(".out").click(function(){a.close(1)})},show:function(){this.ui.dialog.animate({top:0})},
close:function(){var a=this;this.ui.dialog.animate({top:-this.height+"px"},function(){a.ui.wrap.remove();WE.Dialog.remove(a.id)})},onclose:function(){return!0}};
