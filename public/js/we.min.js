...
a)},setPos:function(a,c){this.ui.wrapper.css({top:a+"px",left:c+"px"})}};return function(b){return new a(b)}}()});WE.extend(WE.markdown,{defaultPrefix:"```",format:function(a){if(String(a).length>0){var b="",b=this.preFormat(a);return b=this.urlFormat(b)}else return a},preFormat:function(a){if(a.length>0){var b=a.split(this.defaultPrefix).length-1;if(b!=0&&b%2==0){var c=0;return a.replace(RegExp(this.defaultPrefix,"g"),function(){c++;return c%2==0?"</pre>":"<pre>"})}else return a}else return!1},urlFormat:function(a){return a.replace(/\n/gi,"<br/>").replace(/(http|https):\/\/[\w\.\/\:\?\&\=\#\-\_]+/gi,function(a){return'<a href="'+
a+'" target="_blank">'+a+"</a>"})}});WE.Dialog=function(a){var b=null;if((this.id=a.id)&&WE.Dialog.getItem(this.id))return b=WE.Dialog.getItem(this.id),b.isRepeat=!0,b.isRepeat;this.title=a.title||"";this.html=a.html;this.width=a.width||200;this.height=a.height||60;this.wrap=$(WE.Dialog.html.replace("<%=title%>",this.title));this.ui={wrap:this.wrap,mask:this.wrap.find(".we-mask"),dialog:this.wrap.find(".we-dialog"),context:this.wrap.find(".we-dialog-context")};this.init()};WE.Dialog.html='<div class="we-dialog-box">\t\t<div class="we-mask" style="opacity:0" ></div>\t\t<div class="we-dialog" style="opacity:0">\t\t\t<div class="we-dialog-title">\t\t\t\t<h1><%=title%></h1>\t\t\t\t<a href="javascript:;" class="we-dialog-close out"></a>\t\t\t</div>\t\t\t<div class="we-dialog-context we-loading"></div>\t\t</div>\t</div>';
