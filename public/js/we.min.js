...
a)},setPos:function(a,c){this.ui.wrapper.css({top:a+"px",left:c+"px"})}};return function(b){return new a(b)}}()});WE.extend(WE.markdown,{defaultPrefix:"```",preInnerTxt:null,preOutTxt:null,splitText:function(a){var b=RegExp(this.defaultPrefix+"[\\s\\S]*?"+this.defaultPrefix,"g");this.preOutTxt=a.split(b);this.preInnerTxt=a.match(b)},join:function(){this.fullTxt=this.preOutTxt;if(this.preInnerTxt!=null)for(var a=0;a<this.fullTxt.length;a++)this.fullTxt.splice(2*a+1,0,this.preInnerTxt[a]);return this.fullTxt.join("")},format:function(a){if(String(a).length>0){a=WE.kit.removalHtmlTag(a);this.splitText(a);if(this.preOutTxt!=
null)for(a=0;a<this.preOutTxt.length;a++)this.preOutTxt[a]=this.lineFormat(this.preOutTxt[a]);if(this.preInnerTxt!=null)for(a=0;a<this.preInnerTxt.length;a++)this.preInnerTxt[a]=this.preFormat(this.preInnerTxt[a]);a=this.join();a=this.urlFormat(a)}return a},preFormat:function(a){if(a.length>0){var b=0,c=RegExp(this.defaultPrefix,"g"),a=a.replace(RegExp(this.defaultPrefix+"\r"),this.defaultPrefix);return a.replace(c,function(){b++;return b%2==0?"</pre>":"<pre>"})}else return a},urlFormat:function(a){return a.replace(/(http|https):\/\/[\w\.\/\:\?\&\=\#\-\_]+/gi,
function(a){return'<a href="'+a+'" target="_blank">'+a+"</a>"})},lineFormat:function(a){return a.replace(/\n/gi,"<br/>")}});WE.Dialog=function(a){var b=null;if((this.id=a.id)&&WE.Dialog.getItem(this.id))return b=WE.Dialog.getItem(this.id),b.isRepeat=!0,b.isRepeat;this.title=a.title||"";this.html=a.html;this.width=a.width||200;this.height=a.height||60;this.wrap=$(WE.Dialog.html.replace("<%=title%>",this.title));this.ui={wrap:this.wrap,mask:this.wrap.find(".we-mask"),dialog:this.wrap.find(".we-dialog"),context:this.wrap.find(".we-dialog-context")};this.init()};WE.Dialog.html='<div class="we-dialog-box">\t\t<div class="we-mask" style="opacity:0" ></div>\t\t<div class="we-dialog" style="opacity:0">\t\t\t<div class="we-dialog-title">\t\t\t\t<h1><%=title%></h1>\t\t\t\t<a href="javascript:;" class="we-dialog-close out"></a>\t\t\t</div>\t\t\t<div class="we-dialog-context we-loading"></div>\t\t</div>\t</div>';
