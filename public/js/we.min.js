...
a[0].releaseCapture();this.hideMask();b.unbind("mousemove",this.events.move);b.unbind("mouseup",this.events.up);this.onblur()},stop:function(){this.removeMask()},checkBoundary:function(b,a){var d=this.option.range;$("#movebox").text(d[3]+"-"+this.height);b=b>d[2]-this.width?d[2]-this.width:b;a=a>d[3]-this.height?d[3]-this.height:a;b=b<d[0]?d[0]:b;a=a<d[1]?d[1]:a;return{x:b,y:a}},setResize:function(){var a=this;if(!this.option.isResize)return this.option.moveElement.find(".mx-dialog-change").hide(),
!1;this.resize=new WE.kit.Resize({dragElements:this.option.moveElement.find(".mx-dialog-change"),sizeElement:this.option.moveElement});this.resize.onfocus=function(){a.showMask();a.setMaskIndex()};this.resize.onblur=function(c,d,e,f){a.width=c;a.height=d;a.top=f;a.left=e;a.onResize();a.hideMask()};this.resize.init()},setRange:function(a,c,d,e){this.option.range=[a,c,d,e]},setRangeWindow:function(){var a=this.option.patchTop,c=this.option.patchLeft,d=$(window).width()-this.option.patchWidth,e=$(window).height()-
...
c=new WE.api.AtModel,d=new WE.Controller(c);d.update=function(a){b.showList(a.data)};c.addObserver(d);c.searchFirends(a||"")}};WE.Dialog=function(a){var b=null;if((this.id=a.id)&&WE.Dialog.getItem(this.id))return b=WE.Dialog.getItem(this.id),b.isRepeat=!0,b.isRepeat;this.title=a.title||"";this.html=a.html;this.width=a.width||100;this.height=a.height||50;this.wrap=$(WE.Dialog.html.replace("<%=title%>",this.title));this.ui={wrap:this.wrap,dialog:this.wrap.find(".we-dialog"),context:this.wrap.find(".we-dialog-context")};this.init()};WE.Dialog.html='<div class="we-dialog-box">\t\t<div class="we-mask"></div>\t\t<div class="we-dialog" style="opacity:0">\t\t\t<div class="we-dialog-title">\t\t\t\t<h1><%=title%></h1>\t\t\t\t<a href="javascript:;" class="we-dialog-close out"></a>\t\t\t</div>\t\t\t<div class="we-dialog-context">\u52a0\u8f7d\u4e2d...</div>\t\t</div>\t</div>';
...
WE.Dialog.prototype={constructor:WE.Dialog,init:function(){var a=this;this.id&&WE.Dialog.setItem(this.id,this);document.body.appendChild(this.ui.wrap[0]);this.ui.context.width(this.width);this.ui.context.height(this.height);this.updatePosition();this.ui.wrap.delegate(".out","click",function(){a.close(1)})},append:function(a){this.ui.context.css({width:"auto",height:"auto"});this.ui.context.html(a);this.width=this.ui.dialog.width();this.height=this.ui.dialog.height();this.updatePosition()},updatePosition:function(){this.ui.dialog.css({top:"30%",
left:"50%",marginTop:-(this.height/2-10)+"px",marginLeft:-(this.width/2)+"px"})},show:function(){this.ui.dialog.animate({marginTop:"+=10px",opacity:1},300)},close:function(){var a=this;this.ui.dialog.animate({marginTop:"-=10px",opacity:0},300,function(){a.ui.wrap.remove();WE.Dialog.remove(a.id)})},onclose:function(){return!0}};
