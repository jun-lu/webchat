...
"",c=new Date(a),d=new Date;a=d.getFullYear();var e=d.getMonth()+1,d=d.getDate(),f=c.getFullYear(),g=c.getMonth()+1,h=c.getDate(),k=[f,g,h],c=[c.getHours(),c.getMinutes(),c.getSeconds()];f==a?g==e?b=h==d?"Today "+c.join(":"):1==h-d?"Yesterday "+c.join(":"):k.splice(1,3).join("-")+" "+c.join(":"):g!=e&&(b=k.splice(1,3).join("-")+" "+c.join(":")):b=k.join("-")+" "+c.join(":");return b},sortFormat:function(a){return this.format(a,"yyyy-MM-dd")},longFormat:function(a){return this.format(a,"yyyy-MM-dd hh:mm:ss")},
...
(a="webkitHidden");return document[a]},titleRoll:function(a,b){var c=a.split("");setInterval(function(){c.push(c[0]);c.shift();document.title=c.join("")},b)}});WE.extend(WE.cookie,{getCookie:function(a){for(var b=document.cookie.split(";"),c=0,d=b.length,e=null,f=null;c<d;c++)if(e=b[c].split("="),a==e[0].replace(/(^\s*)|(\s*$)/g,"")){f=e[1];break}return f},setCookie:function(a,b){var c=a+"="+escape(b);document.cookie=c}});WE.extend(WE.ui,{ok:function(a,b){var c=this.tip({context:a,okbtn:!1,cancelbtn:!1,type:"success",timeout:b||3E3,title:"\u63d0\u793a"});c.ui.close.focus();return c},alert:function(a,b){this.alert.tip&&(this.alert.tip.option.callback=null,this.alert.tip.close());var c=this.tip({context:a,cancelbtn:!1,type:"notice",callback:b,title:"\u63d0\u793a"});c.ui.ok.focus();return this.alert.tip=c},confirm:function(a,b){if(this.confirm.tip)return this.confirm.tip;var c=this,d=this.tip({context:a,closebtn:!1,type:"notice",
