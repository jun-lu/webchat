...
location.reload():c.errorTip(a.msg)};d.addObserver(e);d.updateMail(a,b)},errorTip:function(a){this.ui.tips.text(a).removeClass("hidden")}};
WE.userPage.home={tmpl:'<li>\t\t\t\t<div class="avatar pull-left">\t\t\t\t\t<a href="/user/<%=_id %>">\t\t\t\t\t\t<img src="<%=avatar %>">\t\t\t\t\t</a>\t\t\t\t</div>\t\t\t\t<div class="info pull-left">\t\t\t\t\t<a href="/user/<%=_id %>"><%= name == "" ? "(\u672a\u8bbe\u7f6e\u6635\u79f0)" : name %></a>\t\t\t\t\t<span><%=summary == "" ? "&nbsp;" : WE.kit.cutOff(summary,30)  %></span>\t\t\t\t</div>\t\t\t</li>',noTmpl:"<li>\t\t\t\t<span>No contects...</span>\t\t\t</li>",init:function(){this.ui={list:$("#contacts")};
this.loadContacts()},loadContacts:function(){var a=this,b=new WE.api.UserModel,c=new WE.Controller;c.update=function(b){b=b.data;0==b.code&&(0<b.result.length?a.appends(b.result):$(a.noTmpl).appendTo(a.ui.list))};b.addObserver(c);b.getContactList()},appends:function(a){var b="",c=0;if(0<a.length){for(var d=a.length;c<d;c++)b+=WE.kit.tmpl(this.tmpl,a[c].to);$(b).appendTo(this.ui.list)}}};
