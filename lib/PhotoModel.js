...
PhotoModel.prototype.getNetworkImage = function(url, callback){
	...
	if(!protocol){
		callback(new WebStatus(-1).toJSON());
		return false;
	}
...
PhotoModel.prototype.copyPhoto = function( photo, newPath, options, callback ){
	...
	promise.then(function( photo, features ){
		...
		var newphoto = new Photo(photo.fileName, photo.format, "5125b7ac93a2f5dd48000005", "51c6bbfc00aad2b948000002", feature);
		//newphoto._id = "123456";
		newphoto.subdirectory = newPath || newphoto.subdirectory;
		
		new PhotoModel().insert(newphoto, function( status ){
			if( status.code == 0 ){
