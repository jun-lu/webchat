...
//mongodb
ChatModel.prototype.findChats = function( selecter, limit ){

	//chatModel.find( {roomid:room.id });
    ...
    this.getDb().open(function (error, client) {
      ...
      collection.find( selecter ).sort({time:-1}).limit( limit ).toArray(function(err , items ){
      		client.close();
