		...
		if( err ){
			...
			if( data == null ){
				...
				for(var i=0; i< data.length; i++){
					list.push( User.factory( data[i] ).getInfo() );
				}
...
UserModel.inquireOne = function( selecter, callback){
 ...
 */
 ...
 UserModel.find_id = function( id , callback){
 	...
 	this.inquireOne( {_id:new ObjectID(id)} , callback);
 }
...
UserModel.updateSummary = function( id, summary, callback){
...
/** 
	查询多个用户
*/
UserModel.getMultiple = function( ids, callback){
...
UserModel.addRoomPassword = function(id, roomid, password, callback){
 ...
 */
 ...
 UserModel.createOauthUser = function( mail, name, pwd, oauth_options, callback ) {
...
UserModel.createOauthUser = function( user, callback ) {
...
UserModel.findOauth = function(from, openid, callback){
